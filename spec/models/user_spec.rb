...
RSpec.describe User, type: :model do
  let(:user) { build(:user) }
  let(:user_with_real_boy_tasks) { build(:user_with_real_boy_tasks) }
  ...
  it 'has two tasks' do
    expect(user_with_real_boy_tasks.tasks.length).to eq(2)
  end

  it 'returns tasks due today' do
    task = user_with_real_boy_tasks.tasks.first
    task.update(due_date: DateTime.now)

    expect(user_with_real_boy_tasks.due_today.length).to eq(1)
  end
end
